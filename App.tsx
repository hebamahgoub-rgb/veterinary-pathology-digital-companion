...
import { hashToView, tabForView, viewToHash } from './navigation';
...
export default function App() {
  ...
  const [currentTab, setCurrentTab] = useState<NavigationTab>(() =>
    tabForView(hashToView(window.location.hash))
  );
  ...
  // Keep app navigation synchronized with the browser URL and history.
  useEffect(() => {
    if (!window.location.hash) {
      window.history.replaceState(null, '', viewToHash({ type: 'home' }));
    }

    const handleHashChange = () => {
      const nextView = hashToView(window.location.hash);
      setCurrentView(nextView);
      setCurrentTab(tabForView(nextView));
    };
  ...
  const handleTabSelect = (tab: NavigationTab) => {
    switch (tab) {
      case 'home':
        ...
        break;
      case 'search':
        handleNavigate({ type: 'search' });
        break;
      case 'videos':
        handleNavigate({ type: 'videos' });
        break;
      case 'saved':
        handleNavigate({ type: 'saved' });
        break;
    }
  };
  ...
  const handleSelectLesson = (lessonId: string) => {
    handleNavigate({ type: 'lesson', lessonId });
  };

  const handleSelectTopic = (topicId: string) => {
    handleNavigate({ type: 'topic_detail', topicId });
  };
