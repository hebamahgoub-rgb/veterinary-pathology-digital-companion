...
export interface CellInjuryLayoutProps {
...
export const CellInjuryLayout: React.FC<CellInjuryLayoutProps> = ({
  header,
  lessonTitle,
  sections,
  currentIndex,
  ...
  return (
    <div
      id="segmented-cell-injury-container"
    ...
    >
      ...
      >
        ...
        <div className="text-center text-xs text-slate-500 font-medium hidden md:block">
          Section <strong className="text-amber-900">{currentIndex + 1}</strong> of{' '}
        ...
        {nextSection ? (
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-amber-800 hover:bg-amber-900 active:bg-amber-950 text-white text-xs sm:text-sm font-semibold transition-colors shadow-xs cursor-pointer ml-auto"
          >
            <div className="text-right min-w-0">
              <span className="text-[10px] text-amber-200 block font-normal leading-none mb-0.5">
                Next section
              </span>
              <span className="truncate block max-w-[180px] sm:max-w-[200px]">
          ...
          >
            ...
            <span>Complete Lesson · General Pathology</span>
          </button>
        )}
      </div>
      ...
      {isLastSection && (
        <div id="final-segment-footer-group" className="space-y-4 pt-2">
          ...
          <div className="bg-emerald-50/80 border border-emerald-200/90 rounded-2xl p-4 sm:p-5 flex items-center justify-between gap-4">
            ...
            <button
              onClick={onCompleteLesson}
              className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white border border-emerald-300 text-emerald-900 text-xs font-semibold hover:bg-emerald-100 transition-colors shadow-2xs whitespace-nowrap cursor-pointer"
            >
